...
int main()
{
	...
	if(fp==NULL)
	{
		printf("PLEASE CHECK THE DATABASE FILE \n");
	}
	fscanf(fp,"%d",&max1);
	for(i=0;i<max1;i++)
	{
	...
	while(strcmp(option[0],"yes")==0)
	{
		...
		if(strcmp(option[1],"6")==0)
		{
			...
			if(strcmp(option[1],"1")==0)
			{
				...
				{
					printf("THE GIVEN AUTHOR WROTE %d DISTINCT BOOKS IN THE GIVEN LIST\n \n",n);
				}
			}
			if(strcmp(option[1],"2")==0)
			{
		...
		if(strcmp(option[1],"10")==0)
		{
			node_type* p;
			 ...
			 {
			 	printf("*******************************************************\n\n");
			 	printf("******THE SYMMETRIC LIST DIFFERENCE LIST IS EMPTY******\n\n");
			 	printf("*******************************************************\n\n");
			 }
			 else
			 {
		...
		if(strcmp(option[1],"12")==0)
		{
			...
			while(tempo!=NULL)
			{
					printf("THE BOOK NAME IS: %s\n",tempo->book_name);
					printf("THE AUTHOR NAME IS: %s \n",tempo->author);
					printf("THE ACCESS NUMBER: %ld\n",tempo->access_number);
					printf("THE COPY NUMBER: %d\n",tempo->copy_number);
					printf("ISBN NUMBER: %ld \n",tempo->isbn_number);
					printf("THE PRICE OF THE BOOK: %d Rs\n",tempo->price);	
					printf("THE PUBLLISHER OF THE BOOK: %s\n",tempo->publisher);
					printf("---------------------------------------------------\n");
					tempo=tempo->next;
			}
		}
	...
	while(lptr!=NULL)
	{
		if(strcmp(lptr->author,author)==0)
		{
			...
			else
			{
				...
				else if(strcmp(temp->book_name,lptr->book_name)!=0)
				{
					n++;
					temp=lptr;
				}
