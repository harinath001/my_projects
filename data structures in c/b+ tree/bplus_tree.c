...
#include<string.h>
#include<math.h>
#define max 4 //maximum number of children
...
int main()
{
	...
	data_type data,d[mid];
	int done1=1,k;
	char option[10];
/*	for(k=20;k<=43;k++)
	{
		data.anum=k;
		strcpy(data.book,"dspd2");
...
/*	for(k=10;k<=50;k++)
	...
	//root=delete(root,10,&lptr);
	//root=delete(root,2,&lptr);
	//root=delete(root,4,&lptr);
	...
	while(done1==1)
	{
		...
		/*else if(strcmp(option,"-1")==0)
		{
			...
			printf("the key yoou wwant is %d %d\n",((((root->i_node).x[1]))->i_node).key[max-2],((((root->i_node).x[1]))->i_node).key[max-1]);
	        getch();
		}*/
	}
...
node* insert(node *root,node** lptr,data_type data)
{
	node* p=root,*q,*temp,*tempo;
		int u,v=0;
	node* n1,*n2;
	int anum=data.anum,j,k,mini,median,flag2=1;
	data_type data2,d[mid];
	if(p==NULL)
	{
		n1=make_node(1);
		n2=make_node(0);
	...
	else
	{
		while(p->type==0)
		{
			...
			if(k<0)
			{
				printf("ERROR IN INSERT BECAUSE KEY VALUE IS GOING LESS THAN MIN\n");
				getch();
			}
			q=p;
			p=(p->i_node).x[k];
	...
	flag2=1;
			if((q->i_node).x[0]!=NULL)
				{
						...
						if(k==1&&(((q->i_node).x[0])->d_node).active<=max&&mini!=-1&&p==NULL)
				{
					printf("the 1st node not filled\n");
					getch();
					if((((q->i_node).x[0])->d_node).active==max)
					{
						temp=make_node(1);
						(temp->d_node).data[0]=(((q->i_node).x[0])->d_node).data[max-1];
						((((q->i_node).x[0])->d_node).data[max-1]).anum=-1;
						(((q->i_node).x[0])->d_node).active=max-1;
						mini=insert_into_array((temp->d_node).data,data,1);
						(temp->d_node).active=2;
					}
					 if((((q->i_node).x[0])->d_node).active<max)
					{
						(q->i_node).key[0]=anum+1;
						mini=insert_into_array((((q->i_node).x[0])->d_node).data,data,1);
		...
		if(p!=NULL && flag2==1)
		{
			...
			//getch();
			mini=insert_into_array((p->d_node).data,data,0);
		/*	if(anum==15)
		{
			printf("the data which is going to insert is %d\n",data.anum);
			printf("after while loop value of mini is %d\n",mini);
			printf("the k value is %d",k);
			getch();}*/
				
				 if((p->d_node).active<max&&mini!=-1)
				{
					mini=insert_into_array((p->d_node).data,data,1);
					((p->d_node).active)++;
				}
				else if(mini!=-1)
				{
					if((p->d_node).active==max)
					{
						
						if(p==(q->i_node).x[max-1])
						{
							/*if(anum==37)
							{
							...
							if(anum<((p->d_node).data[mid+1]).anum)
							{
								data2=(p->d_node).data[max-1];
								((p->d_node).data[max-1]).anum=-1;
								mini=insert_into_array((p->d_node).data,data,1);
								data=data2;
								anum=((p->d_node).data[mid+1]).anum;
							}
							
							v=0;
							for(u=mid+1;u<=max-1;u++)
							{
								d[v]=(p->d_node).data[mid+v+1];
								((p->d_node).data[mid+v+1]).anum=-1;
								((p->d_node).active)--;
								v++;
							}
							d[v]=data;
							...
							for(n=median+1;n<max;n++)//check here n<max or n<max-1
							{
						...
						else if(anum>((p->d_node).data[mid+1]).anum)
						{
							//printf("AT PRESENT THE KEY IN COUSIN IS %d\n",(q->i_node).key[k]);
							//getch();
						/*	if(anum>=18)
							{
								printf("checking for cousin\n");
							...
							(q->i_node).key[k]=((p->d_node).data[mid+1]).anum;
...
int insert_into_array(data_type a[],data_type data,int mode)
{
	...
	k=-1;
			for(j=0;j<=max-1&&flag==1&&flag2==1;j++)
			{
...
node* insert_inode(int key,node* parent,node* q,node* temp,node* root)
{
	...
	{
	//	printf("the parent node last key is %d\n",(parent->i_node).key[max-2]);
	//	getch();
		
		if((parent->i_node).key[max-2]==-1)
		{
		...
		/*	printf("now we see the new keys array formed\n");
			...
			while((parent->i_node).key[k]!=-1&&k<max-1)
			{
...
node* delete(node* root,int anum,node** lptr)
{
	...
	int j,k,m,n,flag=1,mini,merge=0;
	data_type data[min+1];
	...
	else
	{
			...
			while(p->type==0)
		{
			...
			if(k<0)
			{
				printf("ERROR IN DELETION BECAUSE KEY VALUE IS GOING LESS THAN MIN\n");
				getch();
			}
			q=p;
			p=(p->i_node).x[k];
	...
	else 
	{
		...
		else
		{
			...
			if(k==max-1)
			{
				if((p->d_node).active<=min)
				{
					if((((q->i_node).x[k-1])->d_node).active<=(max-min))
					{
						//n=0;
						for(j=0;j<min;j++)
						{
							mini=insert_into_array((((q->i_node).x[k-1])->d_node).data,(p->d_node).data[j],1);
					...
					else
					{
						...
						mini=insert_into_array((p->d_node).data,(((q->i_node).x[k-1])->d_node).data[j],1);
						((p->d_node).active)++;
						if(mini!=(((q->i_node).x[k-1])->d_node).data[j].anum)
						
						{
							printf("ERROR IN DELETION IN THE CASE K=MAX-1\n");
			...
			else//deltion is not from the last data node
			{
					...
					if((p->d_node).active<=min)
					{
						...
						if(((next->d_node).data[1]).anum==-1)
						{
							...
							else
							{
								...
								{
									mini=insert_into_array((p->d_node).data,(next->d_node).data[0],1);
									((p->d_node).active)++;
				...
				else if((q->i_node).x[k+1]==NULL)
				{
					...
					if((p->d_node).active<=min && k!=0 && (p->d_node).active!=0)
					{
						if((((q->i_node).x[k-1])->d_node).active<=(max-min))
						{
						//n=0;
					...
					else if((p->d_node).active>min)
					{
						(q->i_node).key[k-1]=((p->d_node).data[0]).anum;
					}
					else if((p->d_node).active<=min && k!=0 && (p->d_node).active==0)
					{
						prev=(q->i_node).x[k-1];
						if(prev==NULL)
						{
